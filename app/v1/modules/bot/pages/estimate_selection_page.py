...
class EstimateSelectionPage(BasePage):
    ...
    def search_and_open_estimate(self, estimate_id: str) -> None:
        ...
        4. If a locked dialog appears, dismiss once and wait for open/second lock
    ...
    def _handle_locked_estimate_dialog(self, estimate_id: str) -> None:
        """Dismiss first lock popup; stop only if the lock popup appears again."""
        ...
        self._debug(
            f"First locked estimate dialog dismissed for estimate_id={estimate_id}; "
            "waiting for record to open or a second lock dialog"
        )
        outcome = self._wait_for_estimate_opened_or_locked(expect_locked=True)
        if outcome != "locked":
            return

        self._dismiss_locked_estimate_dialog_if_present()
        raise EstimateLockedError(
            f"Estimate {estimate_id} is still locked after dismissing the lock "
            "dialog; stopping flow"
        )
