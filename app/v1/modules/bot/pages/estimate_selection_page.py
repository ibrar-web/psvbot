...
class EstimateLockedError(RuntimeError):
    pass
...
class EstimateSelectionPage(BasePage):
    ...
    def search_and_open_estimate(self, estimate_id: str) -> None:
        ...
        4. If a locked dialog appears, dismiss it, release all record locks via
           the shared admin-panel flow (same as Estimate History lookup), and
           retry once before giving up
        5. Otherwise wait for the estimate record to open
        """
        ...
        # Step 3: Wait for a not-found warning, locked dialog, or the estimate to open
        ...
        if outcome == "locked":
            self._handle_locked_estimate_dialog(estimate_id)
    ...
    def _handle_locked_estimate_dialog(self, estimate_id: str) -> None:
        """Dismiss the lock dialog, release all record locks via the shared
        admin-panel flow (same pattern used for Estimate History lookup),
        and retry opening the estimate. Give up only if it's still locked
        after that.
        """
        if not self.dismiss_locked_record_dialog_if_present():
            raise EstimateLockedError(
                f"Estimate {estimate_id} is locked by user and the OK dialog "
                "could not be dismissed; stopping flow"
            )

        self._debug(
            f"Estimate {estimate_id} is locked; releasing all record locks and retrying"
        )
        self.unlock_all_records()

        # Re-run the full search (module selection + typing the id) rather
        # than just pressing Enter again — the Record Lock admin panel took
        # us away from the search box, so its prior state can't be trusted.
        self._debug(f"Re-searching for estimate_id={estimate_id} after unlocking records")
        self._select_estimate_module()
        self.wait_for_visible(self.SEARCH_INPUT)
        self.type(self.SEARCH_INPUT, str(estimate_id), clear_first=True)
        self.page.keyboard.press("Enter")
        ...
        outcome = self._wait_for_estimate_opened_or_locked(expect_locked=True)
        if outcome == "locked":
            self.dismiss_locked_record_dialog_if_present()
            raise EstimateLockedError(
                f"Estimate {estimate_id} is still locked after releasing all "
                "record locks; stopping flow"
            )
